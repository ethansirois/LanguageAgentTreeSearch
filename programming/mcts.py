...
def answer(idx, item, max_iters, log_path, is_leetcode, is_math, n, exe, gen, model, print_v, num_items, success,
           cur_func_impl):

    if is_leetcode:
        tests_i = item['visible_tests']
    elif is_math:
        tests_i = ["type(solveMathProblem()) == float"]
    else:
        tests_i =  gen.internal_tests(item["prompt"], model, 4) # ["type(solveMathProblem()) == float"]
...
def run_mcts(
    ...
    print("about to run through the dataset, hopefully it's parallel!")
    ...
    for idx, item in enumerate(dataset):
        answer(idx, item, max_iters, log_path, is_leetcode, is_math, n, exe, gen, model, print_v,
                                   num_items, success, cur_func_impl)

    print("total accuracy: ", str(sum(success) / num_items))
